		...
		for (int i = 0; i < numPlayers; i++)
		{
			...
			if (team != 2)
			{
				...
				if((m_pClient->m_Snap.m_paFlags[0] && m_pClient->m_Snap.m_paFlags[0]->m_CarriedBy == pInfo->m_ClientId) || (m_pClient->m_Snap.m_paFlags[1] && m_pClient->m_Snap.m_paFlags[1]->m_CarriedBy == pInfo->m_ClientId))
				{
					...
					IGraphics::CQuadItem QuadItem(line.x + abs(75.0f - teeRenderInfo.m_Size) / 2.0f - 35.0f * lineHeight / 64.0f, line.y + teeRenderInfo.m_Size / 2.0f - 43 * lineHeight / 64.0f, size/2, size);
...
void CScoreboard::RenderScoreboard(float x, float y, float w, int Team, const char *pTitle)
{
	...
	if(!pTitle)
	{
		...
		if (g_Config.m_ClColorNicks && m_pClient->m_Snap.m_pGameobj && m_pClient->m_Snap.m_pGameobj->m_Flags&GAMEFLAG_TEAMS && Team != TEAM_SPECTATORS)
		{
			if (Team)
			{
				TextRender()->TextColor(0.7f, 0.7f, 1.0f, 1.0f);
	...
	else
	{
		...
		{
			char aBuf[128];
			int Score = Team == TEAM_RED ? m_pClient->m_Snap.m_pGameobj->m_TeamscoreBlue : m_pClient->m_Snap.m_pGameobj->m_TeamscoreRed;
			str_format(aBuf, sizeof(aBuf), "%d", Score);
...
void CScoreboard::OnRender()
{
	...
	if (m_Active == 2 || (g_Config.m_ClDetailedScoreboard && DoScoreBoard && m_Active != 1))
	{
